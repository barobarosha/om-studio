...
export function Layout({ children }: { children: ReactNode }) {
  ...
  const nav = [
    ...
    { to: "/izdeliya", label: "Изделия", title: "Изделия из керамогранита" },
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-[hsl(var(--background))]/95 backdrop-blur border-b border-border">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-3 h-16 overflow-visible">
          <button className="xl:hidden p-2 shrink-0" onClick={() => setMenuOpen(!menuOpen)} aria-label="Меню">
          ...
          <nav className="hidden xl:flex items-center gap-2.5 ml-3 text-[13px] flex-1 min-w-0">
            {nav.map((n) => (
              <NavLink
                key={n.to}
                to={n.to}
                ...
                className={({ isActive }) =>
                  `whitespace-nowrap hover:text-primary transition-colors ${isActive ? "text-primary font-semibold" : "text-foreground/80"}`
                }
              >
                {n.label}
              </NavLink>
          ...
          {/* Поиск: форма всегда w-36, поле раскрывается ВЛЕВО поверх меню — соседние кнопки не двигаются */}
          <form onSubmit={submitSearch} className="hidden md:block ml-auto relative group w-36 h-9 shrink-0">
            <input
              value={searchQ}
              onChange={(e) => setSearchQ(e.target.value)}
              placeholder="Поиск"
              className="absolute right-0 top-0 z-20 h-9 w-36 rounded-md border border-input bg-white pl-3 pr-9 text-sm cursor-text shadow-sm transition-all duration-300 ease-out group-hover:w-64 focus:w-64 focus:outline-none focus:ring-1 focus:ring-ring"
              data-ym="search-input"
            />
            <button className="absolute right-2 top-1/2 -translate-y-1/2 z-30 text-muted-foreground" aria-label="Найти">
              <Search size={16} />
            </button>
          </form>
          <a
            ...
            className="hidden xl:flex shrink-0 items-center gap-2 text-sm font-semibold whitespace-nowrap"
            data-ym="phone-link"
          >
          ...
          <button
            ...
            className="hidden md:block shrink-0 h-9 px-4 rounded-md bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity whitespace-nowrap"
        ...
        {menuOpen && (
          <nav className="xl:hidden border-t border-border bg-background px-4 py-3 flex flex-col gap-1">
            {nav.map((n) => (
              <NavLink key={n.to} to={n.to} className="py-2.5 border-b border-border/50 text-[15px]" onClick={() => setMenuOpen(false)}>
                {n.label}
              </NavLink>
            ))}
            ...
            <button
              onClick={() => { setMenuOpen(false); setLead("callback"); }}
              className="mt-1 h-11 rounded-md bg-primary text-primary-foreground text-sm font-semibold"
            >
              Заказать звонок
            </button>
          </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 grid gap-8 md:grid-cols-4 text-sm">
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-foreground/80">
              ...
              <li><Link to="/izdeliya">Изделия из керамогранита</Link></li>
