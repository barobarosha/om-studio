      ...
      {(collections.data?.length ?? 0) > 0 && (
        <div className="flex flex-wrap gap-2 mb-8">
          {collections.data!.slice(0, 30).map((c) => (
            <Link key={c.id} to={`/catalog?brand=${brand.data!.id}&collection=${encodeURIComponent(c.name)}`} className="px-3 py-1.5 rounded-full border border-border bg-card text-xs hover:border-primary">
              {c.name} · {c.cnt}
            </Link>
          ))}
        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {(data ?? []).map((c) => (
            <Link key={c.id} to={`/catalog?collection=${encodeURIComponent(c.name)}`} className="border border-border bg-card rounded-lg px-4 py-3 hover:border-primary/50 transition-colors">
