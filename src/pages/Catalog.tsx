...
type Multi = "brands" | "countries" | "colors" | "surfaces" | "sizes" | "designs" | "shapes" | "collections";
...
function useFiltersFromUrl() {
  ...
  const filters = {
    ...
    sizes: getList("size"),
    designs: getList("design"),
    shapes: getList("shape"),
    collections: getList("collection"),
    largeFormat: params.get("large") === "1",
    mosaic: params.get("mosaic") === "1",
...
export default function CatalogPage({ forceCategory, forceSale, forceSearch }: { forceCategory?: string; forceSale?: boolean; forceSearch?: boolean }) {
  ...
  const apiInput = useMemo(
    () => ({
      ...
      designs: filters.designs.length ? filters.designs : undefined,
      shapes: filters.shapes.length ? filters.shapes : undefined,
      collections: filters.collections.length ? filters.collections : undefined,
      largeFormat: filters.largeFormat || undefined,
      mosaic: filters.mosaic || undefined,
  ...
  const title = forceSale
    ...
    : search
      ...
      : filters.mosaic
        ? "Мозаика"
        : filters.largeFormat && category === "keramogranit"
  ...
  const facetGroups: { key: Multi; urlKey: string; label: string; items: { value: string; cnt: number }[] }[] = [
    { key: "collections", urlKey: "collection", label: "Коллекция", items: (facets.data?.collections ?? []) as any },
    { key: "designs", urlKey: "design", label: "Дизайн", items: (facets.data?.designs ?? []) as any },
    { key: "shapes", urlKey: "shape", label: "Форма", items: (facets.data?.shapes ?? []) as any },
  ...
  const filtersBody = (
    <div className="space-y-5">
      ...
      <button
        onClick={() => setFilter({ brand: undefined, country: undefined, color: undefined, surface: undefined, size: undefined, design: undefined, shape: undefined, collection: undefined, pmin: undefined, pmax: undefined, stock: undefined })}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {filters.mosaic ? (
        <p className="text-muted-foreground text-sm mb-4 max-w-2xl">Мозаика на сетке и декоративные элементы для стен и полов.</p>
      ) : filters.largeFormat && category === "keramogranit" ? (
        <p className="text-muted-foreground text-sm mb-4 max-w-2xl">Керамогранит больших форматов — плиты от 120×120 см и более.</p>
      ) : category && CATEGORY_TITLES[category] ? (
