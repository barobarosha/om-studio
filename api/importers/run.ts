...
import { norm, slugify, guessDesign, guessShape, cleanText, guessBrandFromName } from "./normalize";
  ...
  try {
    ...
    const rows = items.map((it) => {
      ...
      return {
        ...
        size: it.size ?? null,
        design: it.design ?? guessDesign(it.name, it.collectionName, it.description) ?? null,
        shape: it.shape ?? guessShape(it.name, it.size) ?? null,
    ...
    const updateSet = {
      ...
      design: sql`VALUES(design)`,
      shape: sql`VALUES(shape)`,
